...
import { Employer } from '~~/types/contentful'
const { data: employers } = await useFetch<Employer[]>(`/api/employers`)
</script>
...
<template>
  <div class="rounded-2xl border border-zinc-100 p-6 dark:border-zinc-700/40">
    ...
    <ol class="mt-6 space-y-4">
      <!-- employers -->
      <li v-for="employer in employers" class="flex gap-4">
        <div
        ...
        <dl class="flex flex-auto flex-wrap gap-x-2">
          ...
          >
            {{ new Date(employer.startDate).getFullYear() }}-{{
              employer.endDate
                ? new Date(employer.endDate).getFullYear()
                : 'Present'
            }}{{ employer.note }}
          </dd>
        </dl>
